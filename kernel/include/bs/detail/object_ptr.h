...
/// @brief `object_ptr` impl to be used in function args instead of raw ptrs and `const shared_ptr<T>&`
/// @copyright Anthony Williams https://github.com/anthonywilliams/object_ptr.git
#pragma once
...
template <typename T> struct hash<blue_sky::object_ptr<T>> {
	constexpr size_t operator()(blue_sky::object_ptr<T> const &p) const
		noexcept {
		return hash<T *>()(p.get());
	}
};
...
typename std::enable_if<
	...
	blue_sky::object_ptr<To>>::type
static_pointer_cast(blue_sky::object_ptr<From> p) {
...
typename std::enable_if<
	...
	blue_sky::object_ptr<To>>::type
dynamic_pointer_cast(blue_sky::object_ptr<From> p) {
