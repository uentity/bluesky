...
public:
	...
	type_descriptor()
		:
		bs_ti_(nil_type_info()), creation_fun_(NULL), copy_fun_(NULL), parent_td_fun_(NULL),
	...
	type_descriptor(
		...
		const BS_GET_TD_FUN& parent_td_fn, const std::string& type_name, const std::string& description
	) :
	...
	// express ctor for BlueSky types with type name auto-deducing
	template< class T, class base = nil, int nocopy = 0 >
	type_descriptor()
		:
		bs_ti_(BS_GET_TI(T)), creation_fun_(T::bs_create_instance),
		copy_fun_(extract_copyfn< T, nocopy >::go()), parent_td_fun_(base::bs_type),
		type_name_(bs_type_name< T >())
	{}
